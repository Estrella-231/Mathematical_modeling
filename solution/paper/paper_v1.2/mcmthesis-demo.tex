...
The average interval width $[V_{i,t}^{-}, V_{i,t}^{+}]$ remains relatively narrow (approaching 0), suggesting that Softmax normalization compresses variance and may understate true uncertainty. We treat these bounds as a conservative lower bound and recommend bootstrap-based intervals for future refinement.
...
random forest models to predict contestant performance based on fan votes (Mfan) and judge scores
...
Global feature importance does not reveal nonlinear or instance-level effects: it summarizes average impact and can mask opposing effects across subgroups or over time. We therefore use SHAP, Shapley Additive Explanations, to attribute each prediction to individual features. Unlike global importance metrics that aggregate contributions across all observations, SHAP values provide individualized explanations by decomposing each prediction into additive feature contributions. This methodology is grounded in cooperative game theory, where the Shapley value quantifies each feature's marginal contribution across all possible feature coalitions, satisfying three fundamental properties: local accuracy (feature contributions sum to the model output), missingness (absent features receive zero attribution), and consistency (monotonicity with respect to feature effects). The contribution of feature $x_i$ for a given observation is defined by the Shapley value over the model $f$:
\begin{equation}\label{eq:shap_single}
\text{SHAP}_i = \sum_{S \subseteq X \setminus \{x_i\}} \frac{|S|! (k - |S| - 1)!}{k!} \left[ f(S \cup \{x_i\}) - f(S) \right]
\end{equation}
Here $S$ runs over all subsets of features excluding $x_i$, $X$ is the full feature set, $k$ is the number of features, and the weight $\frac{|S|! (k - |S| - 1)!}{k!}$ is the usual Shapley kernel ensuring fair attribution across all possible feature coalitions. The term $f(S \cup \{x_i\}) - f(S)$ measures the marginal contribution of adding feature $x_i$ to coalition $S$, capturing how the feature alters model predictions conditional on different feature subsets. A positive $\text{SHAP}_i$ means that feature $i$ raises the predicted outcome; a negative value means it lowers it. We compute SHAP for both the fan and judge Twin Random Forest models and aggregate over contestant-weeks. This yields a local, additive explanation for each contestant-week and allows comparison of nonlinear effects across features in a consistent, interpretable way, enabling us to identify threshold effects, interaction patterns, and heterogeneous impacts that aggregate importance metrics would obscure. The resulting SHAP values thus support both instance-level interpretation (e.g., why a given contestant received high or low fan support) and population-level contrast between judge and fan drivers.
...
\\\textbf{Superstars} (39\%, high--high): avg.\ placement 3.0. 
\\\textbf{Tech Giants} (21\%, high--low): e.g.\ Sabrina Bryan (S5) eliminated Week 6 despite high scores. 
\\\textbf{Fan Favorites} (3.6\%, low--high): controversy cluster; e.g.\ Bobby Bones (S27), Jerry Rice (S2), Bristol Palin (S11). 
\\\textbf{Underdogs} (36\%, low--low): avg.\ placement 10.1.
...
\textbf{Hyperparameter stability.} Ridge's regularization parameter $\alpha$ exhibits a broad optimal plateau ($\alpha \in [10, 100]$) where test $R^2$ remains stable at 0.45 with variance $< 0.001$. This two-order-of-magnitude stability eliminates hyperparameter tuning concerns. Performance degrades gracefully outside this range: overfitting when $\alpha < 1$ (train-test gap $> 0.10$), underfitting when $\alpha > 1000$ ($R^2 < 0.40$). Figure~\ref{fig:ridge_hyperparam} illustrates this stability across the full parameter range, with the optimal plateau clearly visible between $\alpha = 10$ and $\alpha = 100$. Hyperparameter sensitivity index $S_\alpha = 0.0010$ confirms extremely low sensitivity, suitable for production deployment.

\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{../../figures/ridge_sensitivity_paper/ridge_hyperparameter_stability.pdf}
\caption{Ridge hyperparameter stability: test $R^2$ versus $\alpha$.}
\label{fig:ridge_hyperparam}
\end{figure}

\textbf{Feature perturbation sensitivity.} Gaussian noise injection (0.1$\sigma$, 0.2$\sigma$, 0.5$\sigma$) reveals feature importance hierarchy: \texttt{judge\_rank\_in\_week} (most sensitive, $R^2$ drop: 0.082 at 0.5$\sigma$), \texttt{relative\_judge\_score} (moderate, drop: 0.034), \texttt{cumulative\_average} (minimal, drop: 0.008). Rankings remain stable under alternative perturbation methods, validating that weekly decisions depend on current performance rather than historical trends.

\textbf{Sample size sensitivity.} Performance saturates at 80\% of training data (2,800 samples), achieving 97.8\% of maximum $R^2$. Test $R^2$ improves from 0.38 (20\% sample) to 0.44 (80\%) and 0.45 (100\%). Early saturation indicates model capacity matches data complexity. Variance across 10 random subsamples remains low (std $< 0.02$), confirming reproducibility.

\textbf{Noise robustness.} Three noise scenarios assess real-world reliability, as illustrated in Figure~\ref{fig:ridge_noise}: (1) Gaussian noise injection shows 5.1\% $R^2$ drop at 10\% noise level, 11.8\% drop at 20\% noise, with graceful linear degradation; (2) Missing data testing reveals mean imputation achieves $R^2 = 0.423$ (6.0\% drop) at 20\% missingness, substantially outperforming median imputation (10.9\% drop) and zero-filling (16.0\% drop); (3) Outlier injection at 5\% rate with 3$\sigma$ magnitude causes only 4.2\% performance drop. Ridge's $L_2$ regularization provides inherent robustness through down-weighting of extreme residuals.
...
\textbf{Cross-validation stability.} Performance remains consistent across partitioning schemes: 5-fold ($R^2 = 0.448 \pm 0.021$), 10-fold ($R^2 = 0.450 \pm 0.018$), and leave-one-season-out ($R^2 = 0.447 \pm 0.023$), with variation $< 0.015$. Minimal gap between random k-fold and leave-one-season-out ($\Delta R^2 < 0.003$) confirms generalization to unseen seasons.

\textbf{Summary.} Sensitivity analysis establishes Ridge regression as production-ready: broad optimal $\alpha$ plateau (Figure~\ref{fig:ridge_hyperparam}) eliminates tuning complexity, tolerance to 10\% noise and 20\% missingness with $< 7\%$ degradation (Figure~\ref{fig:ridge_noise}) ensures real-world applicability, and feature ranking stability validates substantive interpretations. Recommended deployment: default $\alpha = 50$, mean imputation for missing data, quality monitoring thresholds (noise $< 10\%$, missingness $< 20\%$), and 3$\sigma$ outlier detection.
